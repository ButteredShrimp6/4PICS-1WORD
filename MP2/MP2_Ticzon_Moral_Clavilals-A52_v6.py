...
##            f=open('PlayerData.txt','r')
    ...
    def Frame1(self):
        ...
        self.LevelNumber = Label(Topframe, text = (Window.Level+1),font = 'Arial 15 bold',bg = 'Royal blue')
    ...
    def FrameShowLetters(self):
        ...
        self.wordanswer = [None for x in range(len(Window.picfiles[Window.Level]))]
        r = 0
        c = 0
        for i in range(len(Window.picfiles[Window.Level])):
        ...
        DeleteButton.grid(column = len(Window.picfiles[Window.Level])+2, row = 0)
    ...
    def keyboardchange(self):
        ...
        x = Window.picfiles[Window.Level]
        letters = sample(string.ascii_uppercase,12 - len(Window.picfiles[Window.Level]))
        for k in x.upper():
            n = randint(0,11)           
            letters.insert(n,k)
    ...
    def changeImage(self):
        ...
        self.FrameShowLetters()
        self.keyboardchange()
        self.pics.config(file=Window.picfiles[Window.Level]+".png")
    ...
    def hint(self):
        word = Window.picfiles[Window.Level].upper()
    ...
    def check(self,*args):
        ...
        try:
            for i in self.wordanswer:
                word = word+i['text']
            if word == Window.picfiles[Window.Level].upper():
                if Window.Level==50:
                    Window.Level=0
                Window.Level+=1
                showframe.destroy()
    ...
    def CoinCheck(self):        
        if (int(self.CoinsLeft['text'])-10)==0:
            self.BLight_bulb.config(state = DISABLED)
            self.PassButton.config(state = DISABLED)

        elif (int(self.CoinsLeft['text'])-2)==0:
            self.BLight_bulb.config(state = DISABLED)
            self.PassButton.config(state = DISABLED)

    def UserSave(self):
        with open('PlayerData.txt','w'):
            f.write('Level:{}.{}'.format())
